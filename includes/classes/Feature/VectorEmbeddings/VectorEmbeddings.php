		...
		if ( Utils\is_epio() ) {
			try {
				...
				if ( ! method_exists( $epio, 'is_service_available' ) ) {
					throw new \Exception( esc_html__( 'You need to update the ElasticPress plugin to use this feature.', 'elasticpress-labs' ) );
				}
			} catch ( \Exception $e ) {
				$status->code      = 2;
			...
			if ( isset( $epio ) && ! $epio->is_service_available( 'vectorEmbeddings' ) ) {
				$status->message[] = wp_kses_post(
					sprintf(
						/* translators: %s: Upgrade URL */
						__( 'Your current ElasticPress.io plan does not include the vector embeddings service. <a href="%s" target="_blank">Upgrade now!</a>', 'elasticpress-labs' ),
